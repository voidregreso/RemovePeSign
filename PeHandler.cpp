...
extern "C" BOOL IsInvalidPe(LPVOID pBase)
{
	if (NULL == pBase)
		return TRUE;
...
extern "C" LPVOID MapPeFile(LPCWSTR pwzFile, PLARGE_INTEGER lpFileSize)
{
	if (wcslen(pwzFile) <= 0)
		return NULL;
...
extern "C" void UnmapPeFile(LPVOID pBase)
{
	if (NULL != pBase)
		UnmapViewOfFile(pBase);
}

extern "C" BOOL GetPeSignOffsetAndSize(LPVOID pBase, LPDWORD lpOffset, LPDWORD lpSize)
{
...
extern "C" BOOL RemovePeSign(LPVOID pBase, PLARGE_INTEGER lpFileSize, LPCWSTR pwzNewFile)
{
	BOOL bRet = FALSE;
